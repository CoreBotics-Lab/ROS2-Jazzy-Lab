...
class action_server_node_class : public rclcpp::Node{
    public:
        action_server_node_class() : Node("counter_action_server"){
            ...
            this->action_server_ = rclcpp_action::create_server<CounterAction>(
                ...
                // Callback for when a goal is accepted. This starts the execution.
                [this](const std::shared_ptr<GoalHandle> goal_handle) {
                    // CRITICAL: The long-running task MUST be run in a separate thread.
                    // If we run the task in this callback, it will block the executor,
                    // and the node will be unable to process other events, like cancellation requests.
                    std::thread([this, goal_handle]() {
                        this->execute_callback(goal_handle);
                    }).detach(); // Detach the thread to let it run independently ("fire and forget").
                });
        }
