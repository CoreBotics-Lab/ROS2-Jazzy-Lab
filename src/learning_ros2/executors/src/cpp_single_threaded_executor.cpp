...
class SingleThreadedDemoNode: public rclcpp::Node{
    public:
        SingleThreadedDemoNode(): Node("single_threaded_demo_node"){
            ...
            this->fast_timer_ = this->create_timer(500ms,
                [this]()->void{this->fast_timer_callback();},
                this->fast_group_);
            this->slow_timer_ = this->create_timer(2000ms,
...
int main(int argc, char * argv[]) {
    ...
    std::shared_ptr<SingleThreadedDemoNode> node_instance = nullptr;
    ...
    try {
        ...
        node_instance = std::make_shared<SingleThreadedDemoNode>();
        
        // --- THE SHIFT MANAGER ---
        // Explicitly defining the SingleThreadedExecutor to perfectly match the Python demo
        rclcpp::executors::SingleThreadedExecutor executor;
        executor.add_node(node_instance);
        RCLCPP_INFO(log, "Spinning with SingleThreadedExecutor...");
        executor.spin(); 
