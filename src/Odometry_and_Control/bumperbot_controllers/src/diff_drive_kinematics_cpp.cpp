...
DiffDriveKinematicsClass::DiffDriveKinematicsClass()
  ...
  if (wheel_radius_param.get_type() ==
          rclcpp::ParameterType::PARAMETER_NOT_SET ||
      wheel_separation_param.get_type() ==
          rclcpp::ParameterType::PARAMETER_NOT_SET) {
    throw std::runtime_error(
        "A required parameter was not set! Please load a config file.");
  }
  ...
  RCLCPP_INFO(this->get_logger(),
              "Successfully loaded parameters from config file:");
  RCLCPP_INFO(this->get_logger(), "-> wheel_radius: %.3fm",
              this->wheel_radius_);
  RCLCPP_INFO(this->get_logger(), "-> wheel_separation: %.3fm",
...
void DiffDriveKinematicsClass::callback_cmd_vel(const Twist::SharedPtr msg) {
  ...
  double left_wheel_speed =
      (v_linear - (v_angular * this->wheel_separation_ / 2.0)) /
      this->wheel_radius_;
  double right_wheel_speed =
      (v_linear + (v_angular * this->wheel_separation_ / 2.0)) /
      this->wheel_radius_;

  // EXPLICIT STEP 1: Allocate a brand-new heap box for this specific runtime
  // cycle.
  ...
  // EXPLICIT STEP 3: Dereference with the asterisk (*) to match the publisher's
  // standard const reference value signature.
